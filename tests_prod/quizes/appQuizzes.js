...
describe("APP_QUIZZES", function () {
    ...
    function execute(k) {
        baseurl = null;
        it("TEST_FOR_BASIC_GET_" + quizzes[k].toUpperCase() + "_QUIZZES", function (done) {
            this.timeout(30000);
            baseurl = url + quizzes[k] + "/subjects/";
            chakram.get(url + quizzes[k] + "/subjects").then(function (result) {
