...
AppEmployerModel.prototype.updateAppEmployer = function (id, employerInfo, callback) {
    this.dbMySQL.query('UPDATE app_employer SET ? WHERE id = ' + id, employerInfo, function (err) {
        callback(err, employerInfo);
    });
};
AppEmployerModel.prototype.createAppEmployer = function (employerInfo, callback) {
    this.dbMySQL.query("insert into app_employer SET ?", employerInfo, function (err, results) {
        if (results) {
            employerInfo.id = results.insertId;
        }
        callback(err, employerInfo);
    });
};
AppEmployerModel.prototype.find = function (params, callback) {
...
AppEmployerModel.prototype.findOne = function (params, id, callback) {
    var sql = this.tableOnly;
    if (params.type) {
        sql = this.tableWithDependenciesSQL;
    }
...
AppEmployerModel.prototype.create = function (data, callback) {
    var self = this;
    if (data.company) {
        company.update(data.company.id, data.company, function (err, companyData) {
            ...
            delete data.company;
            self.createAppEmployer(data, function (err, result) {
...
AppEmployerModel.prototype.update = function (id, data, callback) {
    var self = this;
    var params = {};
        ...
        if (result) {
            if (data.company) {
                company.update(data.company.id, data.company, function (err, companyData) {
                    ...
                    data.companyid = companyData.data.id;
                    delete data.company;
                    self.updateAppEmployer(id, data, function (err, result) {
...
AppEmployerModel.prototype.remove = function (id, callback) {
    var sql = 'DELETE FROM app_employer WHERE id = ' + id;
    this.dbMySQL.query(sql, callback);
};
AppEmployerModel.prototype.search = function (params, callback) {
    var sql = this.tableOnly;
    if (params.type) {
        sql = this.tableWithDependenciesSQL;
    }
...
module.exports = AppEmployerModel;
